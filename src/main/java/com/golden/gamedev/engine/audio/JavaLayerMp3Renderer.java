...
public class JavaLayerMp3Renderer extends BaseAudioRenderer {
	...
	protected void playSound(final URL audiofile) {
		Thread thread = new Thread() {
			...
			public void run() {
				try {
					JavaLayerMp3Renderer.this.Mp3player = new Player(
					        new BufferedInputStream(audiofile.openStream()),
					        FactoryRegistry.systemRegistry()
					                .createAudioDevice());
					
					JavaLayerMp3Renderer.this.Mp3player.play();
				}
				catch (IOException e) {
	...
	public int getStatus() {
		if (this.Mp3player != null) {
			...
			return (this.Mp3player.isComplete()) ? BaseAudioRenderer.END_OF_SOUND
			        : super.getStatus();
		}
	...
	public static void main(String args[]) {
		...
		String music1 = "file:///D:\\Data\\backup\\NetBeansProjects\\Mario\\src\\music\\"
                        + "music 1_sel.mp3";
